...
Partitioning in Cassandra is implemented by assigning tokens, denoted as $tk$, which are numbers computed by a some  function (usually hash function), to keys and storing the data in node for which token is in its token range. 
...
Token value $91$ belongs to the node $1$, and since \RF{3}, then the value is replicated on $2$ more nodes, which are on counter wise positions, thus on node $2$ and node $3$, which are \emph{secondary replicas} of the token. Tokens define the units of replication -- the smallest segments of the data that can be replicated. 
