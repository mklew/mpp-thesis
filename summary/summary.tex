...
%Moreover isolation level could be configurable per each transaction. 
...
% \paragraph{Reducing contention}
% Further work is required to reduce possible contentions during the commit procedure. We think we could limit the contention if we could abort transaction at the early stage of the commit. 
...
% \lwt executes only when a condition is 
% \paragraph{Formal proof}
% We have implemented and tested \mpt algorithm in Cassandra, which shows that the concept of multi-partition transactions with read-committed isolation level works under our tests. 


% \paragraph{Effective conflict resolution functions}
