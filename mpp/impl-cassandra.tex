...
\section{Indended use of transactions}
...
\section{New consistency levels}
...
\section{Extending binary protocol}
...
\section{MPP Private Transaction Storage}
\emph{Private Transaction Storage} implementation which uses private \emph{memtables} to store transaction's modifications. It maps transaction id to its data \code{TransactionData}. Transaction data stores \emph{mutations} mapped by keyspace, table and \textbf{full key}, thus storage has information about what keys are modified in transaction. Each owned transaction item has corresponding entry in transaction data map.
...
\begin{enumerate*}
\item \code{COMMON_TX_ITEMS} -- conflict on same transaction items
