...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4.5cm,
  ...
  \node[main node] (S) [below of=I] {Setup};
  \node[main node] (PRE) [below of=S] {Prepare};
  \node[main node] (PRO) [below of=PRE] {Propose};
  \node[main node] (REP) [right of=PRE] {Repair};
  \node[main node] (C) [below of=PRO] {Commit};
  \node[main node] (R) [left of=C] {Rollback};

  \path[every node/.style={font=\sffamily\small,
  		fill=white,inner sep=1pt}]
  	% Right-hand-side arrows rendered from top to bottom to
  	% achieve proper rendering of labels over arrows.
    (I) %edge [loop above] node {PrRd/-, PrWr/-} (I)
        edge [bend left=0] node[right=1mm] {$\mathit{setup()}$} (S)
    %     edge [bend left=40] node[right=1mm] {BusRd/Flush} (S)
    (S) edge [bend left=30] node[right=1mm] {$\mathit{prepare()}$} (PRE)        
        edge [bend right=40] node[right=1mm] {$\mathit{rolled\_back()}$} (R)        
    
    (PRE) edge [loop above=30] node[right=1mm]  {$\mathit{not\_promised()}$} (PRE)
      	  edge [bend left=30] node[right=1mm]  {$\mathit{propose()}$} (PRO)
    	  edge [bend left=40] node[right=1mm] {$\mathit{repair()}$} (REP)  
    	  edge [bend right=40] node[right=1mm] {$\mathit{rolled\_back()}$} (R)
    %     edge [bend left=40] node[right=1mm] {BusRdX/Flush} (I)
    (PRO) edge [bend left=30] node[right=1mm]  {$\mathit{commit()}$} (C)    	  
    %     edge [bend left=40] node[right=1mm] {BusRdX/Flush} (I)    
    (REP) edge [bend left=30] node[right=1mm] {$\mathit{repaired()}$} (PRE)       
...
Let us consider an example of transaction affecting different replica groups. Let us assume \RF{3} and transaction for keys: $k1$, $k2$, $k3$, $k4$.
...
Pre prepare phase provides a foundation for further phases and provides guarantees that if a replica is in the phase, then it is eligible to participate in a paxos round, and its private data is eligible for commit. Whether commit occurs or not depends on the results of the next phases. Two main components in the phase are: \begin{enumerate*}
\item transaction registers in a transaction index, in order to participate in a paxos round,
\item transaction performs data consistency check, in order to have the local data ready for commit.
\end{enumerate*}
...
Since Transaction Index is accessed by many concurrent transactions, it has to be exclusively acquired by transaction before any operation that changes its state takes place. 
Locking, or other concurrency control mechanism can be used to guarantee that state of Transaction Index stays consistent.


\emph{Paxos round id}, used to find Paxos State data at each node locally, is a synthetic id and is different at each replica during \mpt round. 
We put forward the claim that difference in id between replicas does not have negative effect, as long as conflicting transactions are put into same round. 
...
Only single node has two modifications which illustrates that we don’t have quorum of replica nodes in consistent state. However paxos phases depend on quorum semantics, which means that quorum of nodes within replica group, such as this one is able to proceed and whole algorithm makes progress. 
...
Any replica affected by transaction can transition to rollback phase just after setup phase, but only from following phases:
