...
Replicas advance in phase after successful \emph{transitions}. The algorithm is done when quorum of each replica group is in end phase or when a timeout occurs, in which case commit procedure is aborted. Figure \ref{fig:phasesBasicPath} presents the transitions from idle phase to committed phase assuming successful transitions of each $n\in \text{\nodesTx}$.
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3.5cm,
  ...
  \node[main node] (A) [below of=PRO] {Accepted};
  \node[main node] (REP) [right of=PRE] {Repair};
  \node[main node] (C) [below right of=A] {Commit};
  ...
  \path[every node/.style={font=\sffamily\small,
    ...
    (I) %edge [loop above] node {PrRd/-, PrWr/-} (I)
        edge [bend left=0] node[right=1mm] {$\mathit{setup\_transition()}$} (S)
    ...
    (PRE) edge [loop above=30] node[right=1mm]  {$\mathit{prepare\_again\_transition()}$} (PRE)
      	  edge [bend left=30] node[right=1mm]  {$\mathit{propose\_transition()}$} (PRO)
    ...
    (PRO) edge [bend left=30] node[right=1mm]  {$\mathit{propose\_transition()}$} (A)
...
\item transition to prepared from repairing, which can transition to:
	\begin{itemize}
		...
		\item transition to setup phase, when repair is done, but \node{i} was forced by quorum to transition into repair phase
	\end{itemize}
