...
\subsection{Tokens}
% Napisać, że Token to najmniejsza jednostka replikacji. Token należy do token range, 
% TODO zostało to pokazane na rysunku gdzieś tam

Each key has a token, which is a number computed by a hash function, which belongs to the token ring. \mpt is not dependent on particular hash function. In case of \emph{JVM} tokens are of type \emph{long} which occupies 8 bytes of memory. Keys are not restricted by any type, thus for example a key can be a 100 character long string which requires around $232$ bytes\footnote{exact size depends on \emph{JVM} options used}. Size of the key is then 29 times size of a token. Complex keys can have even bigger size. Token is deterministic in size and in most cases occupies less memory than key itself.

Ranges of token ring are divided among nodes in cluster. Division and assignment of ranges to nodes is a topology. Given topology replicas of particular key can be identified by computing token from a key and then using topology it is known which nodes are assigned to a token range that includes the token. As a consequence, any node in the cluster can point to replicas of particular key or its token.
